...
An ETL pipeline that extracts tournament data from a CSV files, applies transformation logic with pandas, and loads the processed output to BigQuery for downstream analytics — orchestrated with Apache Airflow.
 
## Overview
This pipeline automates the end-to-end flow of FIFA World Cup 2026 data:
 
* **Extract**: Pulls data from a CSV files (teams, venues, matches, referees, squads, match events)
